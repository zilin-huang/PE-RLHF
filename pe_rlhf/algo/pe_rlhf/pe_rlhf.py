...
from pe_rlhf.algo.pe_rlhf.visual_model import VisualConstrainedSACModel
from pe_rlhf.algo.sac_lag.sac_lag_model import ConstrainedSACModel
from pe_rlhf.algo.sac_lag.sac_lag_policy import UpdatePenalty
from pe_rlhf.utils.replay_buffer import MyReplayBuffer
...
from pe_rlhf.algo.sac_lag.sac_lag import SACLagTrainer, validate_config
from pe_rlhf.algo.sac_lag.sac_lag_policy import SACPIDConfig, SACPIDPolicy, get_dist_class
...
# PE-RLHF Romve the Lagrangian Multiplier from SACLag. Instead, it directly minimizes the takeover cost
PE_RLHFConfig = merge_dicts(SACPIDConfig,
                         {
                             ...
                             "il_agent_coef": 0.,  # new code
                             "il_expert_coef": 0.,  # new code
                             "no_cql": False,  # new code
                         })
...
def sac_actor_critic_loss(policy, model, _, train_batch):
    ...
    policy.actor_loss = actor_loss + self_regularization_loss + bc_loss # NEW
    ...
    policy.self_regularization_loss = self_regularization_loss  # NEW
    policy.bc_loss = bc_loss    # NEW
...
# The stats function is usually used to record and report various statistics and performance metrics during the training process
# Copied from sac_pid_policy.py, this file adds this to enable the tracking of self_il_loss and bc_loss statistics
def stats(policy, train_batch):
    return {
        ...
        "self_il_loss": tf.reduce_mean(policy.self_regularization_loss), # NEW
        "bc_loss": tf.reduce_mean(policy.bc_loss),  # NEW
...
PE_RLHFPolicy = SACPIDPolicy.with_updates(name="PE_RLHFPolicy",
                                       make_model=build_sac_model,
                                       get_default_config=lambda: PE_RLHFConfig,
                                       postprocess_fn=postprocess_trajectory,
                                       stats_fn=stats,  # NEW
                                       loss_fn=sac_actor_critic_loss)

PE_RLHFTrainer = SACLagTrainer.with_updates(name="PE_RLHFTrainer",
                                         default_config=PE_RLHFConfig,
                                         default_policy=PE_RLHFPolicy,
                                         get_policy_class=lambda config: PE_RLHFPolicy,
