...
from pe_rlhf.utils.config import baseline_eval_config, baseline_train_config
from pe_rlhf.utils.human_in_the_loop_env import HumanInTheLoopEnv

sys.path.append(os.path.dirname(os.path.realpath(__file__)))
sys.path.append(os.path.dirname(os.path.dirname(os.path.realpath(__file__))))
from easydict import EasyDict
from pe_rlhf.algo.gail.exp_saver import Experiment

from pe_rlhf.algo.gail.mlp import Policy, Value
...
create -n pe_rlhf-gail python version=3.7
1. pip install loguru imageio easydict tensorboardX pyyaml  stable_baselines3 pickle5
2. conda install pytorch==1.7.1 torchvision==0.8.2 torchaudio==0.7.2 cudatoolkit=11.0 -c pytorch
...
expert_data_path = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'human_traj_100_new.json')
...
class Learner:
    ...
    def _collect_samples(self):
        ...
        for i in range(self.buffer_length):
            ...
            total_episode_position += [info[idx]["current_position"] for idx in range(self.env_num)]
        ...
        return {"episode_reward_mean": total_episode_reward / done_num,
                ...
                "episode_position_mean": np.mean(total_episode_position)}
    ...
    def evaluation(self, evaluation_episode_num=30):
        ...
        while episode_num < evaluation_episode_num:
            ...
            position.append(info["current_position"])
            state = next_state
            episode_reward += r
    ...
    def train(self, is_train):
        ...
        metrics["episode_position"] = sample_result["episode_position_mean"]
