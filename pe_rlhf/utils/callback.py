...
class PE_RLHFCallbacks(DrivingCallbacks):
    def on_episode_start(
            self, *, worker: RolloutWorker, base_env: BaseEnv, policies: Dict[str, Policy], episode: MultiAgentEpisode,
            env_index: int, **kwargs
    ):
        ...
        episode.user_data["value_takeover"] = 0  # NEW
        episode.user_data["position"] = [] # NEW
    ...
    def on_episode_step(
        ...
        if info is not None:
            ...
            episode.user_data["value_takeover"] += 1 if info["value_based_takeover"] else 0  # NEW
            episode.user_data["position"].append(info["current_position"])  # NEW
    ...
    def on_episode_end(
        ...
        episode.custom_metrics["value_takeover_rate"] = float(episode.user_data["value_takeover"] / episode.length)  # NEW
        episode.custom_metrics["value_takeover_count"] = float(episode.user_data["value_takeover"])  # NEW

        episode.custom_metrics["position_max"] = float(np.max(episode.user_data["position"]))
        episode.custom_metrics["position_mean"] = float(np.mean(episode.user_data["position"]))
        episode.custom_metrics["position_min"] = float(np.min(episode.user_data["position"]))
    ...
    def on_train_result(self, *, trainer, result: dict, **kwargs):
        ...
        result["value_takeover"] = np.nan  # NEW
        ...
        if "value_takeover_count_mean" in result["custom_metrics"]:  # NEW
            result["value_takeover"] = result['custom_metrics']["value_takeover_count_mean"]  # NEW

        if "position_mean" in result["custom_metrics"]:
            result["position"] = result["custom_metrics"]["position_mean"]  # NEW
...
class ILCallBack(PE_RLHFCallbacks):
    def on_train_result(self, *, trainer, result: dict, **kwargs):
        result["success"] = np.nan
        result["crash"] = np.nan
        result["out"] = np.nan
        ...
        if "evaluation" in result:
            ...
            if "value_takeover_count_mean" in eval["custom_metrics"]:
                result["value_takeover"] = eval['custom_metrics']["value_takeover_count_mean"]

            if "position_mean" in eval["custom_metrics"]:
                result["position"] = eval["custom_metrics"]["position_mean"]
