...
class RecorderEnv(Wrapper):
    ...
    def on_episode_step(self, r, info):
        ...
        if info:
            ...
            self.user_data["current_position"].append(info["current_position"])  # NEW
    ...
    def get_episode_result(self):
        ...
        ret["position_max"] = np.max(self.user_data["current_position"]) if ep_len > 0 else 0.0  # NEW
        ret["position_mean"] = np.mean(self.user_data["current_position"]) if ep_len > 0 else 0.0  # NEW
        ret["position_min"] = np.min(self.user_data["current_position"]) if ep_len > 0 else 0.0  # NEW
        return ret
